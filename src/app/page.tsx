...
import { HomeFacultySection, HomeFaqSection, HomeReviewsSection } from "@/components/home/HomeDynamicSections";
import { PublicFooter } from "@/components/PublicFooter";
import { PublicHeader } from "@/components/PublicHeader";
import { applyHomePageData, applyTopCoursesMarkup, buildAboutVideoMarkup, buildAchieveOfferMarkup, buildCategoryChipsMarkup, buildCategoryStripMarkup, buildHeroShowcaseMarkup, buildTrendingStripMarkup, buildWhyKrLogicsMarkup, stripHeroLeftStats } from "@/lib/home-page-content";
...
export default function Home() {
  ...
  const pageSections = useMemo(() => {
    ...
    const heroShowcaseMarkup = settings
      ? buildHeroShowcaseMarkup(settings.banner_images, settings)
    ...
    const pageWithTopCourses = homeData?.top_courses?.length
      ? applyTopCoursesMarkup(dynamicPageMarkup, homeData.top_courses)
      : dynamicPageMarkup;
    const pageWithHeroShowcase = pageWithTopCourses.replace(
    ...
    const trendingStrip = settings ? buildTrendingStripMarkup(settings.trending_links) : trendingStripMarkup;
    const achieveOfferStrip = settings ? buildAchieveOfferMarkup(settings) : examCategoriesMarkup;
    const whySection = settings ? buildWhyKrLogicsMarkup(settings) : whyKrLogicsMarkup;
    ...
    const pageWithWhySection = pageWithAboutLayout.replace(
      "<!-- COURSES -->",
      `${whySection}\n\n<div class="divider"></div>\n\n<!-- COURSES -->`,
    );
    const facultyMarker = "<!-- FACULTY -->";
    ...
    const [mockBlock, afterFacultyMarker = ""] = `${mockMarker}${afterMockMarkup}`.split(facultyMarker);
    const [, afterTestimonialsMarker = ""] = afterFacultyMarker.split(testimonialsMarker);
    const [afterFaqMarkup = ""] = afterTestimonialsMarker.split(faqMarker);
    const [afterContactMarkup = ""] = afterFaqMarkup.split(contactMarker);
    const rawAfterContact = `${contactMarker}${afterContactMarkup}`;
  ...
  return (
    <>
      <main className="kr-logics-reference-home" onClick={handlePageClick}>
        ...
        <div dangerouslySetInnerHTML={{ __html: pageSections.beforeMockMarkup }} />
        <KRLogicsVideos videos={videoItems} />
        <div className="divider" />
        <div dangerouslySetInnerHTML={{ __html: pageSections.mockBlock }} />
        ...
        <HomeFaqSection faqs={homeData?.faqs ?? []} />
